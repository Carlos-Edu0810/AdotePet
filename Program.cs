...
// adocao.RegistrarAdocao(pessoa[6], animal[6], out string status);
...
void Relatorios()
{
    ...
    switch (opcoesMenu)
    {
        ...
        case 2:
            ...
            Console.WriteLine("4. Listar animais disponiveis para adoção");
            Console.WriteLine("5. Retornar para Relatorios");
            Console.WriteLine("6. Retornar para Menu");
            Console.Write("Selecione a opção desejada: ");
            ...
            switch (opcoesMenu)
            {
                ...
                case 4: animal.ListarAnimais(4); break;
...
void Adocao()
{
    ...
    Console.WriteLine("\nRegistro de adoção");
    Console.Write("\nID do Adotante: ");
    string pessoaInput = input.ReceberTexto(Console.ReadLine());
    int localizarPessoa = pessoa.ListaDePessoas().FindIndex(x => x.Id == pessoaInput);

    Console.Write("ID do Adotado: ");
    string animalAdotado = input.ReceberTexto(Console.ReadLine());
    int localizarAnimal = animal.ListaDeAnimais().FindIndex(x => x.Id == animalAdotado);

    adocao.RegistrarAdocao(pessoa[localizarPessoa], animal[localizarAnimal], out string status);
